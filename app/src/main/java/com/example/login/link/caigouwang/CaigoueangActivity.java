...
public class CaigoueangActivity extends AppCompatActivity {
    ...
    private void init() {
        mCGW = (WebView) findViewById(R.id.link_wb_6);
        //WebView加载web资源
        mCGW.loadUrl("http://www.ccgp-hunan.gov.cn/");
        //覆盖WebView默认使用第三方或系统默认浏览器打开网页的行为，使网页用WebView打开
        mCGW.setWebViewClient(new WebViewClient() {
            @Override
        ...
        WebSettings webSettings = mCGW.getSettings();//支持获取手势焦点，输入用户名、密码或其他
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if ((keyCode == KEYCODE_BACK) && mCGW.canGoBack()) {
            mCGW.goBack();
            return true;
        }
    ...
    protected void onDestroy() {
        if (mCGW != null) {
            mCGW.loadDataWithBaseURL(null, "", "text/html", "utf-8", null);
            mCGW.clearHistory();

            ((ViewGroup) mCGW.getParent()).removeView(mCGW);
            mCGW.destroy();
            mCGW = null;
        }
        super.onDestroy();
