...
public class ZhufanggongjijinActivity extends AppCompatActivity {
    ...
    private void init() {
        ...
        WebSettings webSettings = mZFGJJ.getSettings();//支持获取手势焦点，输入用户名、密码或其他
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if ((keyCode == KEYCODE_BACK) && mZFGJJ.canGoBack()) {
            mZFGJJ.goBack();
            return true;
        }
    ...
    protected void onDestroy() {
        if (mZFGJJ != null) {
            mZFGJJ.loadDataWithBaseURL(null, "", "text/html", "utf-8", null);
            mZFGJJ.clearHistory();

            ((ViewGroup) mZFGJJ.getParent()).removeView(mZFGJJ);
            mZFGJJ.destroy();
            mZFGJJ = null;
        }
        super.onDestroy();
