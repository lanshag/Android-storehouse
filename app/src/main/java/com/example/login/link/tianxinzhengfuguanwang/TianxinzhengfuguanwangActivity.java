...
public class TianxinzhengfuguanwangActivity extends AppCompatActivity {
    ...
    private void init() {
        ...
        WebSettings webSettings = mTXQ.getSettings();//支持获取手势焦点，输入用户名、密码或其他
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if ((keyCode == KEYCODE_BACK) && mTXQ.canGoBack()) {
            mTXQ.goBack();
            return true;
        }
    ...
    protected void onDestroy() {
        if (mTXQ != null) {
            mTXQ.loadDataWithBaseURL(null, "", "text/html", "utf-8", null);
            mTXQ.clearHistory();

            ((ViewGroup) mTXQ.getParent()).removeView(mTXQ);
            mTXQ.destroy();
            mTXQ = null;
        }
        super.onDestroy();
